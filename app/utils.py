...
def get_embedding_function():
    """
    Return an OllamaEmbeddings object, which is used to create vector embeddings from text.
    The embeddings model used is "llama3.1".

    Parameters:
        None.

    Returns:
        OllamaEmbeddings: An OllamaEmbeddings object, which can be used to create vector embeddings from text.
    """
...
PROMPT_USER = """Use the following pieces of retrieved context to answer
...
def query_document(vectorstore, query, api_key):
    ...
    prompt_template = ChatPromptTemplate.from_messages([("system", PROMPT_SYSTEM), ("user", PROMPT_USER)])
    ...
    df = pd.DataFrame([structured_response.dict()], index=['Response'])
