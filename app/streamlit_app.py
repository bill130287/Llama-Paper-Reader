...
# Initialize the user query in session state if it doesn't exist
...
def load_streamlit_page():
    ...
    """
    st.set_page_config(layout="wide", page_title="Help You Read the Document with LLM (llama3 through Groq)!")

    # Design page layout with 2 columns: File uploader on the left, and other interactions on the right.
    col1, col2 = st.columns([0.5, 0.5], gap="large")

    with col1:
        st.header("Input your Groq API key")
        ...
        st.text_input('Query', type='default', key='query', label_visibility="collapsed", disabled=False)
...
if uploaded_file is not None:
    ...
    st.write("Upload Document Processed")
    ...
    if st.button("Generate table"):
        with st.spinner("Generating answer"):
            ...
            answer = query_document(vectorstore = st.session_state.vector_store, 
                                    ...
                                    api_key = st.session_state.api_key) # Initialize the API key in session state if it doesn't exist
